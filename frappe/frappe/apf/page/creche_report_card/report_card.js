frappe.pages["report-card"].on_page_load = function (wrapper) {
    ...
    let mobileKebab = $(`
        <div id="mobile-kebab" style="display:none; position:relative;">
            ...
            <div id="mob-kebab-dropdown" style="
                ...
                <a href="#" id="mob-dl-csv-btn" style="display:block;padding:9px 16px;color:#333;text-decoration:none;border-bottom:1px solid #eee;">Download CSV</a>
    ...
    mobileKebab.find('#mob-dl-csv-btn').on('click', async function(e) {
        e.preventDefault();
        mobileKebab.find('#mob-kebab-dropdown').hide();
        const $a = $(this);
        $a.text('Downloading...');
    ...
    function buildFilterSummaryEl(captureWidth) {
        ...
        fieldDefs.forEach(({ key, label, map, isLink }) => {
            ...
            // get_value() returns the document's internal `name` which is often an ID.
            let val = isLink
    ...
    async function downloadCardsPNG() {
        return new Promise((resolve) => {
            ...
            // 1. Static filter summary (reads from page.fields_dict — always accurate)
    ...
    async function downloadCardsCSV() {
        return new Promise((resolve) => {
            try {
                ...
                link.setAttribute("download", "creche_report_cards.csv");
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
        ...
        <body>
           ...
           <div id="dataModal">
            ...
            <div id="logicModal" style="display: none; position: fixed; z-index: 10000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0, 0, 0, 0.5); backdrop-filter: blur(3px);">
                <div id="logicModalContent" class="modal-content" style="background-color: #fff; margin: 5% auto; padding: 20px; border-radius: 12px; width: 90%; max-width: 900px; max-height: 90vh; overflow: hidden; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.2); position: relative; display: flex; flex-direction: column; animation: slideDown 0.3s ease;">
                    <div style="display: flex; flex-wrap: wrap; gap: 10px; align-items: center; margin-bottom: 15px;">
                        ...
                        <button id="downloadLogicCsvBtn" style="padding: 6px 12px; background-color: #5979aa; color: white; border: none; border-radius: 4px; cursor: pointer;">Download CSV</button>
    ...
    async function fetchDashboardData() {
        const baseUrl = `${BASE_URL}/api/method/frappe.apf.page.creche_report_card.dashboard`;
    ...
    async function handleCardClick(cardId, item, cardElement) {
        ...
        const apiUrl = `${BASE_URL}/api/method/frappe.apf.page.creche_report_card.web_report_card_detail.fetch_card_data?${params.toString()}`;
...
async function getDocName(type, identifier) {
    ...
    $(document).on("click", "#downloadLogicCsvBtn", function(e) {
        e.preventDefault();
        const btn = $(this);
        ...
        try {
            ...
            const link = document.createElement("a");
            link.setAttribute("href", url);
            link.setAttribute("download", "card_logic.csv");
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(url);
        } catch (err) {
            console.error("Error generating Logic CSV", err);
            if (frappe && frappe.msgprint) frappe.msgprint("Error generating CSV.");
        } finally {
...
//         const cardStructure = {
//             Col0: [], // Empty since all cards are now distributed across the 4 main columns
//             Col1: ["cre-983d", "curelichi-99bf", "cresubatt-611c", "crenotsubatt-169a", "daycreope-8fa8", "dayattsub-e0a5", "maxattday-0e61", "attperday-c539"],
//             Col2: ["enrchi-a4c9", "curactchi-9abe", "antdatsub-4b97", "antdatnotsub-07f2", "chimeatak-74d7", "chimeanottak-ea05", "snc-57aa"],
//             Col3: ["chiexithimon-ce95", "chienrthimon-c6c5", "modund-2ce9", "modwas-6828", "modstu-3599", "grofal-02c8", "grofal-975f"],
//             Col4: ["cumenrchi-845b", "cumexichi-3df2", "sevund-55ab", "sevwas-ffdb", "sevstu-3cf6", "grofal-8a20", "pat-f32a"]
//         };
...
//                 cardStructure[colKey].forEach(cardId => {
//                     const card = createCardWithLoader(cardId, cardTitles[cardId] || cardId);
//                     card.style.backgroundColor = columnColors[colKey] || '#ffffff';
//                     column.appendChild(card);
//                 });
//             }
//         }
...
//             // Find the values we need for calculations based on new columns structure
//             allData.forEach(data => {
//                 if (data.data) {
//                     if (data.data.Col1) {
//                         const eligibleItem = data.data.Col1.find(i => i.id === "curelichi-99bf");
//                         if (eligibleItem) {
//                             current_eligible_children = eligibleItem.value;
//                         }
//                     }
//                     if (data.data.Col4) {
//                         const enrolledItem = data.data.Col4.find(i => i.id === "cumenrchi-845b");
//                         if (enrolledItem) {
//                             cumm_enrolled_children = enrolledItem.value;
//                         }
//                     }
//                     if (data.data.Col2) {
//                         const measurementItem = data.data.Col2.find(i => i.id === "chimeatak-74d7");
//                         if (measurementItem) {
//                             child_measurement_taken = measurementItem.value;
//                         }
//                         const activeItem = data.data.Col2.find(i => i.id === "curactchi-9abe");
//                         if (activeItem) {
//                             current_active_children = activeItem.value;
//                         }
//                     }
//                 }
//             });
