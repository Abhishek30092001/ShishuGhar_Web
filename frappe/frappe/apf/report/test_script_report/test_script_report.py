...
def execute(filters=None):
    try:
        ...
        fixed_columns = [
            ...
            {"label": "Avg. Attendance (%)", "fieldname": "avg_attendance", "fieldtype": "Data", "width": 180},
...
def get_report_data(filters):
    try:
        ...
        for row in data:
            ...
            # Format avg_attendance as percentage string
            if row.get('avg_attendance') is not None:
                row['avg_attendance'] = f"{row['avg_attendance']:.2f}%"
            else:
                row['avg_attendance'] = ""
...
def build_main_query(filters, params):
    ...
    # ATTENDANCE AVERAGE PER CHILD
    attendance_avg_subquery = """
    LEFT JOIN (
        SELECT 
        ...
        FROM (
            ...
            WHERE 
                ...
                AND cal.date_of_attendance <= %(end_date_last)s
    ...
    ) attendance_avg ON attendance_avg.childenrolledguid = cee.childenrollguid
    """
    ...
    SELECT
        ...
        ROUND(AVG(attendance_avg.avg_monthly_attendance), 2) AS avg_attendance,
