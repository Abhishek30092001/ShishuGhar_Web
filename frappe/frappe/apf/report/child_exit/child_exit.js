frappe.query_reports["Child Exit"] = {
    "filters": [
		{
			fieldname: "year",
			label: __("Year"),
    ...
    onload: function (report) {
        // Attach on_change event dynamically to clear filter on empty value
        report.filters.forEach(filter => {
            filter.df.on_change = function () {
                if (!this.get_value()) {
                    frappe.query_report.refresh();
                }
            };
