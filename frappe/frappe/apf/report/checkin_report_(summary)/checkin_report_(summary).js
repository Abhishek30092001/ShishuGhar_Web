...
frappe.query_reports["Checkin Report (Summary)"] = {
	filters: [
		{
			fieldname: "year",
			label: __("Year"),
		...
		},
		{
			...
			"options": [
				...
				{ "value": "6", "label": __("All User") },
				{ "value": "7", "label": __("GP") },
				{ "value": "8", "label": __("Creche") },
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (data && data.is_total_row) {
			// Return empty for percentage column in total row
			if (column.fieldname === "above_50m_percentage") {
				return "";
			}
			// Bold all other values in total row
			return `<b>${value}</b>`;
		}

		// Color logic for >50m percentage column
		if (column.fieldname === "above_50m_percentage" && data) {
			const percentage = parseFloat(data.above_50m_percentage); // use raw data
			let color = "";
			...
			if (!isNaN(percentage)) {
				...
				value = `<div style="${styles}">${percentage}%</div>`;
			}
		}
