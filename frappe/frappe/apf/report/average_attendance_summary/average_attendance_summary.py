...
def get_summary_data(filters=None):
    ...
    params = {
        ...
        "end_date": end_date, 
        "creche_age": None,  # Added creche_age parameter
    }
    ...
    level_mapping = {
        ...
        "7": ["tf.state", "tf.district", "tf.block", "tf.gp", "tf.creche","tf.supervisor"],
    }
    ...
    SELECT * FROM (
        ...
        FROM (
            SELECT 
                ...
                IFNULL(opx.E0, 0) + (IFNULL(erc.enroll_children, 0) - (IFNULL(opx.E0, 0)+IFNULL(opx.G0toL25, 0)+IFNULL(opx.25toL50, 0)+IFNULL(opx.50toL75, 0)+IFNULL(opx.75toL100, 0)+IFNULL(opx.E100, 0))) AS E0,
            ...
            LEFT JOIN (
                SELECT cee.creche_id, COUNT(*) AS enroll_children
                FROM `tabChild Enrollment and Exit` cee
                INNER JOIN `tabCreche` cr ON cr.name = cee.creche_id 
                WHERE  (cee.date_of_enrollment <=  %(end_date)s and (cee.date_of_exit IS null or cee.date_of_exit >= %(start_date)s))
                AND (%(partner)s IS NULL OR cee.partner_id = %(partner)s) 
                AND (%(state)s IS NULL OR cee.state_id = %(state)s) 
                AND (%(district)s IS NULL OR cee.district_id = %(district)s)
                AND (%(block)s IS NULL OR cee.block_id = %(block)s)
                AND (%(gp)s IS NULL OR cee.gp_id = %(gp)s)
                AND (%(creche)s IS NULL OR cee.creche_id = %(creche)s)
            ...
            LEFT JOIN(
                    ...
                    FROM (
                         SELECT ca.creche_id, cal.childenrolledguid, SUM(cal.attendance) AS days_attended, COUNT(ca.date_of_attendance) AS eligible_open_days,
                         CASE WHEN COUNT(ca.date_of_attendance) = 0 THEN 0 ELSE (SUM(cal.attendance) / COUNT(ca.date_of_attendance)) * 100.0 END AS catt_per                     
                         FROM `tabChild Attendance` AS ca
                                                ...
                                                AND (%(gp)s IS NULL OR ca.gp_id = %(gp)s)
                                                AND (%(creche)s IS NULL OR ca.creche_id = %(creche)s)                               
                                                GROUP BY ca.creche_id,cal.childenrolledguid
                                            ) AS AttendanceData
                        GROUP BY creche_id
            ...
            LEFT JOIN (
                SELECT creche_id, MIN(daily_attendance) AS min_att, CEIL((SUM(daily_attendance)/ COUNT(*))) AS mean_att, MAX(daily_attendance) AS max_att
                 FROM (
                    SELECT ca.creche_id,ca.date_of_attendance, COUNT(cal.name) AS daily_attendance
            ...
            LEFT JOIN (
                SELECT ca.creche_id, COUNT(ca.name) as open_days_count
                FROM `tabChild Attendance` ca
                WHERE ca.is_shishu_ghar_is_closed_for_the_day = 0
                AND ca.date_of_attendance BETWEEN %(start_date)s AND %(end_date)s
                AND (%(partner)s IS NULL OR ca.partner_id = %(partner)s) 
