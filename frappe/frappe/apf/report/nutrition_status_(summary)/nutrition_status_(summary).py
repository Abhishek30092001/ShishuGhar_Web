...
#     # Add common columns
#     fixed_columns.extend([
#         {"label": "No Change", "fieldname": "no_change", "fieldtype": "Int", "width": 150},
#         {"label": f"Measurements Not Available ({calendar.month_name[final_month]}/{final_year})",
#          "fieldname": "measurements_data_not_available", "fieldtype": "Int", "width": 280}
#     ])
...
#     for row in data:
#         # Calculate percentages and create display strings
#         measurements_taken = row.get('measurements_taken', 0)
       
#         if measurements_taken > 0:
#             for field in ['md_nr', 'sv_nr', 'sv_md', 'nr_md', 'md_sv', 'nr_sv']:
#                 cnt = row.get(f'{field}_cnt', 0)
#                 percent = round((cnt / measurements_taken) * 100, 2) if measurements_taken > 0 else 0
#                 row[f'{field}_display'] = f"{int(cnt)} ({percent:.2f}%)"
#         else:
...
#     select_fields.extend([
#         "SUM(COALESCE(transitions.no_change_cnt, 0)) AS no_change"
#     ])
...
#             SUM(CASE WHEN initial.{selected_indicator} = 2 AND final.{selected_indicator} = 1 THEN 1 ELSE 0 END) AS md_sv_cnt,
#             SUM(CASE WHEN initial.{selected_indicator} = 3 AND final.{selected_indicator} = 1 THEN 1 ELSE 0 END) AS nr_sv_cnt,
#             SUM(CASE WHEN initial.{selected_indicator} = final.{selected_indicator} THEN 1 ELSE 0 END) AS no_change_cnt
#         FROM (
#             SELECT
#                 cee.creche_id AS creche_id,
#                 ad.chhguid,
...
#         {"label": f"Active children till ({month_col_heading}/{initial_year})",
#          "fieldname": "enrolled_children", "fieldtype": "Int", "width": 300},
#         {"label": f"Measurements taken ({month_col_heading}/{initial_year})",
#          "fieldname": "measurements_taken", "fieldtype": "Int", "width": 300},
#         {"label": f"Exited ({month_col_heading}/{initial_year} - {calendar.month_name[final_month]}/{final_year})",
#          "fieldname": "exited_children", "fieldtype": "Int", "width": 300}
#     ]
    
#     # Add transition columns based on selected category
...
#         {"label": f"Measurements Not Available ({calendar.month_name[final_month]}/{final_year})",
#          "fieldname": "measurements_data_not_available", "fieldtype": "Int", "width": 300}
#     ])
...
#     final_date = date(final_year, final_month, calendar.monthrange(final_year, final_month)[1])
#     last_day = calendar.monthrange(initial_year, initial_month)[1]
#     initial_last_day = date(initial_year, initial_month, last_day)  # Corrected line

#     selected_gender = filters.get("gender")
...
#     # Define the base fields for each level
#     level_mapping = {
#         "1": ["p.partner_name"],
#         "2": ["s.state_name"],
#         "3": ["s.state_name", "d.district_name"],
...
#     # Build SELECT fields based on group_by_fields
#     select_fields = []
#     field_map = {
#         "p.partner_name": "p.partner_name AS partner",
#         "s.state_name": "s.state_name AS state",
...
#     # Add transition fields based on selected category
#     selected_category = filters.get("category", "all")
#     if selected_category == "all":
#         select_fields.extend([
#             "SUM(COALESCE(transitions.sv_md_cnt, 0)) AS sv_md_cnt",
#             "SUM(COALESCE(transitions.md_nr_cnt, 0)) AS md_nr_cnt",
#             "SUM(COALESCE(transitions.sv_nr_cnt, 0)) AS sv_nr_cnt",
#             "SUM(COALESCE(transitions.nr_md_cnt, 0)) AS nr_md_cnt",
#             "SUM(COALESCE(transitions.md_sv_cnt, 0)) AS md_sv_cnt",
#             "SUM(COALESCE(transitions.nr_sv_cnt, 0)) AS nr_sv_cnt",
#         ])
#     elif selected_category == "normal":
#         select_fields.extend([
#             "SUM(COALESCE(transitions.md_nr_cnt, 0)) AS md_nr_cnt",
...
#     # Add no change and measurements not taken fields
#     select_fields.extend([
#         "SUM(COALESCE(transitions.no_change_cnt, 0)) AS no_change",
#         "SUM(COALESCE(mt.measurements_taken, 0) - COALESCE(transitions.sv_md_cnt, 0) - COALESCE(transitions.md_nr_cnt, 0) - COALESCE(transitions.sv_nr_cnt, 0) - COALESCE(transitions.nr_md_cnt, 0) - COALESCE(transitions.md_sv_cnt, 0) - COALESCE(transitions.nr_sv_cnt, 0) - COALESCE(transitions.no_change_cnt, 0)) AS measurements_data_not_available"
#     ])
...
#     if params["phases"]:
#         where_conditions.append("FIND_IN_SET(c.phase, %(phases)s)")
#     where_conditions.append("(c.creche_opening_date IS NULL OR ( %(initial_last_day)s IS NOT NULL AND c.creche_opening_date <= %(initial_last_day)s ))")
#     where_conditions.append("((%(cstart_date)s IS NULL AND %(cend_date)s IS NULL) OR (c.creche_opening_date BETWEEN %(cstart_date)s AND %(cend_date)s))")
...
#     # Remove the previous logic that was setting no_change to measurements_taken
#     # as we now have measurements_data_not_available to handle the difference
#     for row in data:
#         measured = row.get('measurements_taken', 0)
#         transitions_sum = sum([
#             row.get('sv_md_cnt', 0),
